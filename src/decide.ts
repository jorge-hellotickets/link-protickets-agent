import type { DecideCtx, Decision, MailMessage } from "./core/types";
 ...
 */
export async function decide(ctx: DecideCtx): Promise<Decision> {
  const { lead } = ctx;
...
// ─── Cold / silence follow-up (status="prospect" or "contacted" wake-up) ───

interface FollowUpStateFields {
...
async function decideOnProspect(ctx: DecideCtx): Promise<Decision> {
  ...
  let siteSnapshot: string | undefined = (data as any).siteSnapshot;
  if (!siteSnapshot || siteSnapshot === "(not available)") {
    const dom = (data as any).domain ?? lead.contactRef;
  ...
  const rendered = promptTemplate
    ...
    .replaceAll("{{category}}", (data as any).category ?? "general")
  ...
  try {
    ...
    console.warn(`[link-protickets:first-send] outreach compose failed for lead ${lead.id} domain=${domain}: ${(err as Error).message}`);
 ...
 */
async function planFollowUp(ctx: DecideCtx, statusForKind: string): Promise<Decision> {
  const { lead, now } = ctx;
  const data = (lead.data as LinkLeadData | null) ?? {};
  const state = (lead.state as (FollowUpStateFields & Record<string, unknown>) | null) ?? {};
  ...
  const subtype: FollowUpSubtype = toComposerSubtype(
    instruction.kind,
    statusForKind,
    followUpState.silenceFollowUpCount,
  );
  ...
  const scheduleAt = new Date(
    Math.max(
      instruction.sendNotBefore.getTime(),
      lastOut + 3 * 86_400_000,
      now.getTime() + 60_000,
    ),
  );
...
function computeNextWakeForFollowUp(
  scheduleAt: Date,
  newCount: number,
  status: string,
): Date | null {
  const nextInstruction = nextFollowUp(
    status,
    { silenceFollowUpCount: newCount, firstOutboundAt: scheduleAt.toISOString(), lastOutboundAt: scheduleAt.toISOString() },
    "Europe/Madrid",
  );
...
async function decideOnWaitingInvoice(ctx: DecideCtx): Promise<Decision> {
  return postDealReply(ctx, "waiting_for_invoice");
}

async function decideOnWaitingPayment(ctx: DecideCtx): Promise<Decision> {
  return postDealReply(ctx, "waiting_for_payment");
}

async function postDealReply(
  ctx: DecideCtx,
  status: "waiting_for_invoice" | "waiting_for_payment",
): Promise<Decision> {
  const { lead, messages, now } = ctx;
  const inbound = latestInbound(messages);
  if (!inbound) return { kind: "noop" };
  ...
  if (legacyProspectId === undefined) return { kind: "noop" };
  ...
  let paymentEvidence: string | null = null;
  if (!deal.paidAt) {
  ...
  const stateObj = (lead.state as Record<string, unknown> | null) ?? {};
  ...
  const stateUpdate: Record<string, unknown> = {};
  if (invoiceType === "proforma") stateUpdate.finalInvoicePending = true;
  else if (invoiceType === "final") stateUpdate.finalInvoicePending = false;
  ...
  const promote =
    ...
    normalizeForEvidenceMatch(inbound.body).includes(
      normalizeForEvidenceMatch(paymentEvidence),
    );
...
async function decideOnPaid(ctx: DecideCtx): Promise<Decision> {
  const { lead, now } = ctx;
  const data = (lead.data as LinkLeadData | null) ?? {};
  const legacyProspectId = data.legacyProspectId;
  if (legacyProspectId === undefined) return { kind: "noop" };
  ...
  if (!deal) return { kind: "noop" };
  if (deal.verifiedAt) return { kind: "close", outcomeKind: "deal_closed" };
    ...
    try {
      ...
      if (audit.ok) {
        await db.linkDeal.update({
          where: { id: deal.id },
          data: { audit: audit as unknown as object, verifiedAt: now },
        });
      ...
      await db.linkDeal.update({
        where: { id: deal.id },
        data: { audit: audit as unknown as object },
      });
    } catch (err) {
      audit = null;
      console.warn(
        `[link-protickets] decideOnPaid audit failed for ${deal.prospect.domain}: ${(err as Error).message}`,
      );
    }
  }

  // Stop nudging after 3 reminders; the human takes over.
  if (deal.remindersSent >= 3) return { kind: "noop" };

  const emailHistory = formatEmailHistory(ctx.messages);
  const useFixRequest = audit !== null && audit.issues[0] !== "missing";
  const subtype: FollowUpSubtype = useFixRequest
    ? "link_fix_request"
    : "publication_nudge";
  const draft = await composeFollowUpEmail({
    mode: "follow_up",
...
// ─── Negotiating / contacted (shared inbound path) ───

interface LinkLeadData {
  legacyProspectId?: number;
  domain?: string;
  targetId?: number;
  emailRoundsSent?: number;
  targetKeywords?: string[] | string;
  category?: string;
}

interface LinkLeadState {
  thread?: ThreadState;
}

async function decideOnNegotiating(ctx: DecideCtx): Promise<Decision> {
  const { lead, messages, now } = ctx;
  ...
  const state = ((lead.state as LinkLeadState | null) ?? {}).thread ?? emptyThreadState();
...
interface MapArgs {
  ...
  inbound: MailMessage;
  emailHistory: string;
  localeCfg: ReturnType<typeof getLocaleConfig>;
  model: string;
  now: Date;
  runtimeConfig: Awaited<ReturnType<typeof getLinkAgentRuntimeConfig>>;
...
async function actionToDecision(args: MapArgs): Promise<Decision> {
  const { action, nextThreadState } = args;
  const stateUpdate = { thread: nextThreadState } as Record<string, unknown>;
  ...
  const nextStatus =
    action.kind === "accept" ? "waiting_for_invoice" : "negotiating";

  return sendableToDecision(action, args, nextStatus, /*close*/ false);
}

async function sendableToDecision(
  action: Action,
  args: MapArgs,
  nextStatus: string,
  shouldClose: boolean,
  ...
  const stateUpdate = { thread: args.nextThreadState } as Record<string, unknown>;
  ...
  let transitionPayload: LinkProticketsDealTerms | undefined;
...
function latestInbound(messages: MailMessage[]): MailMessage | null {
...
function formatEmailHistory(messages: MailMessage[]): string {
  return messages
    .map((m) => {
...
function toBudgetCtx(maxPriceCents: number): BudgetCtx {
 ...
 */
async function fetchSiteSnapshot(domain: string): Promise<string> {
