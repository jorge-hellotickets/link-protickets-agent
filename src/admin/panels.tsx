...
import {
  ...
  listTargets,
  listWaves,
} from "@/src/lib/admin/link-agent/service";
...
type PanelRenderer = (searchParams: URLSearchParams) => Promise<ReactNode>;

export const linkProticketsPanels: Record<string, PanelRenderer> = {
  targets: async () => {
    ...
    return (
      <AdminPageSection>
        <AdminPageHeader
          title="Targets"
          description="Link-building target pages."
        />
  ...
  budgets: async () => {
    ...
    return (
      <AdminPageSection>
        <AdminPageHeader
          title="Budgets"
          description="Monthly link-building budgets per country."
        />
  ...
  discovery: async () => {
    ...
    return (
      <AdminPageSection>
        <AdminPageHeader
          title="Discovery"
          description="Run SERP discovery and manage prospects."
        />
  ...
  waves: async () => {
    ...
    return (
      <AdminPageSection>
        <AdminPageHeader
          title="Waves"
          description="Start outreach waves, send batched emails, and monitor progress by country."
        />
  ...
  deals: async () => {
    ...
    return (
      <AdminPageSection>
        ...
        <DealsPanel
          initialDeals={result.data.deals.map((d) => ({
            ...d,
            audit: (d.audit ?? null) as React.ComponentProps<
              typeof DealsPanel
            >["initialDeals"][number]["audit"],
          }))}
        />
      </AdminPageSection>
    );
