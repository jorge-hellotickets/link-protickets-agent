...
function isValidResponse(raw: unknown): raw is LLMResponse {
  ...
  const invoiceTypeOk =
    ...
    r.invoiceType === "final";
  return (
    typeof r.invoiceDelivered === "boolean" &&
    typeof r.evidence === "string" &&
    invoiceTypeOk
  );
}

export async function detectPaymentTransition(
  input: DetectPaymentInput,
): Promise<DetectPaymentResult> {
  ...
  try {
    ...
    console.warn(
      `[link-agent:payment-detector] GATEWAY_ERROR domain=${input.prospectDomain} msg=${JSON.stringify(msg)}`,
    );
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    console.warn(
      `[link-agent:payment-detector] PARSE_FAILURE domain=${input.prospectDomain} msg=${JSON.stringify(msg)} preview=${JSON.stringify(text.slice(0, 200))}`,
    );
    return { invoiceClassification: null, paymentTransition: null, model, durationMs: Date.now() - startMs };
  }
  ...
  if (!isValidResponse(raw)) {
    console.warn(
      `[link-agent:payment-detector] SHAPE_MISMATCH domain=${input.prospectDomain} keys=${JSON.stringify(Object.keys(raw ?? {}))}`,
    );
    return { invoiceClassification: null, paymentTransition: null, model, durationMs: Date.now() - startMs };
  }
  ...
  if (!raw.invoiceDelivered) {
    console.log(
      `[link-agent:payment-detector] NOT_DELIVERED domain=${input.prospectDomain} durationMs=${durationMs} preview=${JSON.stringify(preview)}`,
    );
  ...
  if (evidence.length === 0) {
    console.warn(
      `[link-agent:payment-detector] EMPTY_EVIDENCE domain=${input.prospectDomain} — invoiceDelivered=true but no evidence, preview=${JSON.stringify(preview)}`,
    );
    return { invoiceClassification: null, paymentTransition: null, model, durationMs };
  }
  ...
  const invoiceType: Exclude<InvoiceType, "none"> =
    raw.invoiceType === "proforma" ? "proforma" : "final";
  if (raw.invoiceType === "none") {
    console.warn(
      `[link-agent:payment-detector] TYPE_CONTRADICTION domain=${input.prospectDomain} — invoiceDelivered=true but invoiceType=none, defaulting to final`,
    );
  }

  console.log(
    `[link-agent:payment-detector] DELIVERED domain=${input.prospectDomain} type=${invoiceType} durationMs=${durationMs} evidence=${JSON.stringify(evidence.slice(0, 120))}`,
  );
  ...
  const paymentTransition =
    invoiceType === "final"
      ? { nextStatus: "waiting_for_payment" as const, evidence }
      : null;
