...
import { type LinkAgentLocale, getLocaleConfig } from "./locale-config";
import type { FollowUpComposerContext, FollowUpSubtype } from "./types";
...
function buildOutreachPrompt(locale: LinkAgentLocale): string {
...
export async function composeOutreachEmail(ctx: OutreachContext, model?: string): Promise<OutreachEmailResult> {
  ...
  return parseGatewayJSON<OutreachEmailResult>(text);
}


// ─── Follow-up Prompts ───

const FOLLOW_UP_INSTRUCTIONS: Record<FollowUpSubtype, { objective: string; tone: string }> = {
  cold_follow_up_1: {
  ...
  link_fix_request: {
    objective:
      "Read the thread and write the natural next message for its current state. Match who spoke last and the last topic, then smoothly include the link-fix request. If this fix was already requested in-thread, briefly frame it as a continuation, not a new ask.",
...
function buildFollowUpPrompt(locale: LinkAgentLocale, subtype: FollowUpSubtype): string {
...
export async function composeFollowUpEmail(ctx: FollowUpComposerContext, model?: string): Promise<OutreachEmailResult> {
  const locale = getLocaleConfig(ctx.locale ?? "es-es");
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    try {
      ...
      if (repaired) {
        try {
          return parseGatewayJSON<OutreachEmailResult>(repaired);
        } catch {
          // fall through to retry
        }
      }
      console.warn(
        `[email-composer] JSON parse failed on attempt ${attempt + 1}: ${
...
function tryRepairJsonStrings(text: string): string | null {
