...
export interface Headroom {
  ...
  thisMonth: { str: string; limitCents: number; spentCents: number; committedCents: number };
  nextMonth: { str: string; limitCents: number; committedCents: number };
 ...
 */
export async function getHeadroom(locale: string): Promise<Headroom> {
  const now = new Date();
  ...
  console.log(
    `[budget] ${locale} headroom: this month ${Math.round(thisHeadroom / 100)}€` +
