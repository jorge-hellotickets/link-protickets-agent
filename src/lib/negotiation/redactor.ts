...
const logFailure = makeLogFailure("redactor");

const REDACTOR_RESPONSE_FORMAT = {
...
export const DEFAULT_REDACTOR_PROMPT = `You are Laura Peñalver, a freelance digital content consultant working for Fint Media. Someone else has already decided the move — your job is ONLY to write the email that executes the decided ACTION, in the locale's register and tone.
...
Identity:
- You are Laura Peñalver. If asked who you work for, the answer is "Fint Media".
...
function isValidShape(raw: unknown): raw is { subject: string | null; body: string | null } {
...
/**
 * Format the action as a compact JSON block for the LLM. Keeping the surface
 * small avoids prompt drift — the redactor sees exactly the decided fields
 * and nothing else. `redirectToArticle: false` is omitted to reduce noise.
 */
  ...
  switch (action.kind) {
    ...
    case "counter":
      return JSON.stringify({
        kind: action.kind,
        price: action.price,
        ...(action.redirectToArticle ? { redirectToArticle: true } : {}),
        ...(action.linkAttribute !== "unknown" ? { linkAttribute: action.linkAttribute } : {}),
      });
    case "terminal":
      return JSON.stringify({ kind: "terminal", reason: action.reason });
    default:
      return JSON.stringify({ kind: action.kind });
  }
}

export async function redact(input: RedactorInput): Promise<RedactorResult> {
  const startMs = Date.now();

  // stall / terminal short-circuit — no LLM call, no send.
  if (input.action.kind === "stall" || input.action.kind === "terminal") {
    return { subject: null, body: null, model: input.model, durationMs: 0 };
  }

  const systemPrompt = input.promptOverride ?? DEFAULT_REDACTOR_PROMPT;
  const userContent = [
    `EMAIL THREAD (oldest first):\n${input.emailHistory}`,
    `LATEST INBOUND:\n${input.inboundBody}`,
    `LATEST INBOUND SUBJECT: ${input.inboundSubject}`,
    `ACTION: ${formatAction(input.action)}`,
    `LOCALE:\n${input.locale.localeSettings}`,
    `CLOSING EXAMPLES: ${input.locale.closingExamples}`,
  ...
  try {
    text = await callAIGateway(userContent, {
      ...
      temperature: 0.4,
      maxTokens: 800,
      responseFormat: REDACTOR_RESPONSE_FORMAT,
    });
  } catch (err) {
  ...
  if (!isValidShape(parsed)) {
    logFailure("SHAPE_MISMATCH", `keys=${JSON.stringify(Object.keys((parsed as object) ?? {}))}`);
    throw new Error("redactor response missing required fields");
  }
