...
const modelsRejectingResponseFormat = new Set();
/** Test hook: clear the per-process rejection cache between cases. */
export function __resetResponseFormatRejectionCache() {
 ...
 */
export async function callAIGateway(prompt, opts = {}) {
    ...
    const postBody = async () => fetch(`${GATEWAY_BASE_URL}/chat/completions`, {
        method: "POST",
        headers: {
    ...
    if (res.status === 400 && "response_format" in body) {
        ...
        console.warn(`[ai-gateway] 400 with response_format — retrying without. model=${modelKey} body=${JSON.stringify(errText.slice(0, 300))}`);
    ...
    if (opts.stream) {
        if (!res.body)
            throw new Error("AI Gateway stream: no response body");
        ...
        if (!content) {
            console.warn(`[ai-gateway] empty stream content. model=${modelKey} finish_reason=${finishReason}${usage ? ` usage=${JSON.stringify(usage)}` : ""}`);
    ...
    if (!content) {
        ...
        console.warn(`[ai-gateway] empty content. model=${modelKey}${detail}`);
 ...
 */
export function parseGatewayJSON(text) {
