 ...
 */
const PRICE_RX = /(?:€|\$|EUR|USD)\s*([0-9][0-9.,]*)|([0-9][0-9.,]*)\s*(?:€|\$|EUR|USD|euros?|dólares?)/gi;
/** Liberal ISO / d-m-y / m/d/y matcher. We only check that named dates line up; false negatives are acceptable. */
const DATE_RX = /\b(\d{4}-\d{2}-\d{2})\b|\b(\d{1,2})[\/.-](\d{1,2})[\/.-](\d{2,4})\b/g;
...
export function validate({ body, action, agreedDate }) {
    const issues = [];
    ...
    if (expectedPrice === null) {
        if (prices.length > 0) {
            issues.push(`body names price(s) ${JSON.stringify(prices)} but action ${action.kind} carries none`);
        }
    }
        ...
        if (bad.length > 0) {
            issues.push(`body prices ${JSON.stringify(prices)} do not all equal action.price=${expectedPrice}`);
        }
    }
    if (agreedDate !== null) {
        ...
        if (mismatches.length > 0) {
            issues.push(`body dates ${JSON.stringify(dates)} include mismatches vs agreedDate=${agreedDate}`);
        }
    }
