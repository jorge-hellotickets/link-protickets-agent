 ...
 */
export function decide(state, signals, budget) {
    const baseNext = {
        ...state,
    ...
    const alreadyCountered = (p) => state.countersMade.includes(p);
    const canCounter = countersCount < MAX_COUNTERS;
    const propose = (proposed) => {
        const safe = round10(proposed);
        if (safe > budget.hardCap)
            return null; // hard cap invariant
        if (alreadyCountered(safe))
            return null; // never same price twice
        if (!canCounter)
            return null;
    ...
    if (price <= budget.hardCap) {
        ...
        if (counter) {
            return {
                ...
                nextState: {
                    ...baseNext,
                    countersMade: [...state.countersMade, counter.price],
                },
            };
        }
        return {
    ...
    if (firstTry) {
        return {
            ...
            nextState: {
                ...baseNext,
                countersMade: [...state.countersMade, firstTry.price],
            },
        };
    }
    ...
    if (secondTry) {
        return {
            ...
            nextState: {
                ...baseNext,
                countersMade: [...state.countersMade, secondTry.price],
            },
        };
    }
    return { action: { kind: "decline" }, nextState: baseNext };
}
//# sourceMappingURL=decisor.js.map