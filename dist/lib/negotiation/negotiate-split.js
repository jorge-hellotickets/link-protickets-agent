 ...
 */
import { decide, emptyThreadState, } from "./decisor";
import { extractSignals } from "./extractor";
import { redact } from "./redactor";
import { validate } from "./validator";
function isSendableAction(action) {
    switch (action.kind) {
        case "stall":
        case "terminal":
            return false;
...
export async function negotiateSplit(input) {
    ...
    console.warn(`[link-agent:negotiate-split] validator rejected first attempt — regenerating once. issues=${JSON.stringify(firstCheck.issues)}`);
    ...
    console.warn(`[link-agent:negotiate-split] validator rejected retry — dropping send. issues=${JSON.stringify(secondCheck.issues)}`);
    return {
        action,
        nextState,
        subject: null,
...
//# sourceMappingURL=negotiate-split.js.map