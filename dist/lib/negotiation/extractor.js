...
const VALID_LINK_ATTRIBUTES = new Set([
    "dofollow",
    "nofollow",
    "unknown",
]);
function coerceSignals(raw) {
    ...
    const r = raw;
    const inboundPrice = r.inboundPrice === null || r.inboundPrice === undefined
        ? null
    ...
    if (typeof r.addedNewInfo !== "boolean")
        throw new Error("addedNewInfo must be boolean");
    if (typeof r.assumedWeWriteArticle !== "boolean")
        throw new Error("assumedWeWriteArticle must be boolean");
    if (!Array.isArray(r.askedQuestions) || !r.askedQuestions.every((q) => typeof q === "string")) {
        throw new Error("askedQuestions must be string[]");
    }
    const askedQuestions = r.askedQuestions;
    if (typeof r.mentionedPlacementType !== "string" || !VALID_PLACEMENTS.has(r.mentionedPlacementType)) {
        throw new Error(`invalid mentionedPlacementType: ${JSON.stringify(r.mentionedPlacementType)}`);
    }
    const mentionedPlacementType = r.mentionedPlacementType;
    ...
    if (typeof linkAttributeRaw !== "string" || !VALID_LINK_ATTRIBUTES.has(linkAttributeRaw)) {
        throw new Error(`invalid linkAttribute: ${JSON.stringify(r.linkAttribute)}`);
    }
    const linkAttribute = linkAttributeRaw;
    return {
        inboundPrice,
        intent,
