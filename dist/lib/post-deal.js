...
import { renderPrompt } from "./negotiation/prompt-template";
...
function isValidResponse(raw) {
    ...
    return (typeof r.reasoning === "string" &&
...
function formatAgreedDate(d, locale) {
    try {
...
export async function composePostDealReply(input, model) {
    ...
    try {
        ...
        console.warn(`[link-agent:post-deal] GATEWAY_ERROR domain=${input.prospectDomain} msg=${JSON.stringify(msg)}`);
        throw err;
    }
    let raw;
    try {
        ...
        const msg = err instanceof Error ? err.message : String(err);
        console.warn(`[link-agent:post-deal] PARSE_FAILURE domain=${input.prospectDomain} msg=${JSON.stringify(msg)} preview=${JSON.stringify(text.slice(0, 200))}`);
        throw err;
    }
    if (!isValidResponse(raw)) {
        console.warn(`[link-agent:post-deal] SHAPE_MISMATCH domain=${input.prospectDomain} keys=${JSON.stringify(Object.keys(raw ?? {}))}`);
        throw new Error("post-deal LLM response missing required fields");
    }
    const durationMs = Date.now() - startMs;
    console.log(`[link-agent:post-deal] OK domain=${input.prospectDomain} rule=${raw.ruleApplied} send=${raw.body !== null} durationMs=${durationMs}`);
    return {
...
//# sourceMappingURL=post-deal.js.map